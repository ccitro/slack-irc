...
class Bot {
  constructor(options) {
    ...
    this.channelRouting = options.channelRouting || [];
  ...
  getSlackChannelForIrcMessage(rawAuthor, rawChannel, text) {
    const channel = rawChannel.toLowerCase();

    const slackChannelName = this.invertedMapping[channel];
    if (!slackChannelName || !rawAuthor) {
        return slackChannelName;
    }

    const author = rawAuthor.toLowerCase();
    const match = _.find(this.channelRouting, mapping => {
        return mapping.author.toLowerCase() === author
            && mapping.sourceIrcChannel.toLowerCase() === channel
            && (new RegExp(mapping.messagePattern)).test(text)
    });

    if (match) {
        return match.destinationSlackChannel;
    }
  ...
  sendToSlack(author, channel, text) {
    const slackChannelName = this.getSlackChannelForIrcMessage(author, channel, text);
    if (slackChannelName) {
